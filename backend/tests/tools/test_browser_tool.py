...
async def test_screenshot_returns_data_url():
    with patch("app.tools.browser_tool.settings") as mock_settings, \
        ...
        mock_manager = mock_manager_cls.return_value
        mock_manager.create_sandbox = AsyncMock(return_value="cnt-123")
        mock_manager.exec_in_sandbox = AsyncMock(side_effect=["OK", "data:image/png;base64,abc"])
        mock_manager.destroy_sandbox = AsyncMock()
...
async def test_navigation_error_returns_message():
    with patch("app.tools.browser_tool.settings") as mock_settings, \
        ...
        mock_manager = mock_manager_cls.return_value
        mock_manager.create_sandbox = AsyncMock(return_value="cnt-123")
        mock_manager.exec_in_sandbox = AsyncMock(side_effect=["OK", "Error: failed to navigate"])
        mock_manager.destroy_sandbox = AsyncMock()
