...
from app.tools.browser_tool import browser_navigate, browser_screenshot, browser_click

@pytest.mark.asyncio
async def test_browser_navigate_sandbox_flow():
    with patch("app.tools.browser_tool.settings") as mock_settings, \
        ...
        mock_manager.exec_in_sandbox = AsyncMock(side_effect=["OK", "Mock Page Content"])
...
async def test_browser_screenshot_sandbox_flow():
    with patch("app.tools.browser_tool.settings") as mock_settings, \
        ...
        mock_manager = mock_manager_cls.return_value
        mock_manager.create_sandbox = AsyncMock(return_value="cnt-123")
        mock_manager.exec_in_sandbox = AsyncMock(side_effect=["OK", "data:image/png;base64,mockdata"])
        mock_manager.destroy_sandbox = AsyncMock()
...
async def test_browser_click_sandbox_flow():
    with patch("app.tools.browser_tool.settings") as mock_settings, \
        ...
        result = await browser_click.ainvoke({"url": "https://example.com", "selector": "button#login"})
