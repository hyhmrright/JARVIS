...
async def test_llm_failover_logic():
    ...
    with patch("app.agent.llm.settings") as mock_settings:
        ...
        # Create a primary LLM that always fails
        primary_llm = MagicMock()
        primary_llm.ainvoke = AsyncMock(side_effect=Exception("API Error"))
        primary_llm.with_fallbacks = lambda x: x[0] # Simplified for test if we want to trace
