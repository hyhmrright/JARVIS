...
from app.rag.retriever import maybe_inject_rag_context, RetrievedChunk
...
async def test_ollama_rag_injection_and_invocation():
    ...
    # 模拟检索到的块
    mock_chunks = [
        RetrievedChunk(document_name="test.txt", content="The secret code is 1234.", score=0.95)
    ]
    
    with patch("app.rag.retriever.retrieve_context", AsyncMock(return_value=mock_chunks)):
        # 注入上下文
        ...
        mock_llm.ainvoke = AsyncMock(return_value=AIMessage(content="The document says the secret code is 1234."))
