...
- **프로덕션급 Docker** — 4계층 네트워크 격리, Traefik 엣지 라우터, 완전한 관측성 스택
...
├── docker-compose.yml          # 프로덕션 오케스트레이션 (4계층 네트워크)
├── docker-compose.override.yml # 개발 오버라이드 (포트 노출, 핫 리로드)
...
스크립트가 설정하는 항목: `POSTGRES_PASSWORD`, `MINIO_ROOT_USER/PASSWORD`, `REDIS_PASSWORD`, `JWT_SECRET`, `ENCRYPTION_KEY`, `GRAFANA_PASSWORD`, `DATABASE_URL`, `REDIS_URL`.
