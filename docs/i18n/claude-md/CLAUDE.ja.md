...
└── pyproject.toml     # ルート開発ツール設定（ruff/pyright/pre-commit）、ランタイム依存関係なし
```
...
Hooks の内容：YAML/TOML/JSON フォーマットチェック、uv.lock 同期、Ruff lint+format、ESLint、Pyright、vue-tsc 型チェック、gitleaks シークレットスキャン、main への直接コミットをブロック。
