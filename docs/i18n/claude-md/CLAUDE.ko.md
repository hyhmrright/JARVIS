...
└── pyproject.toml     # 루트 개발 도구 설정 (ruff/pyright/pre-commit), 런타임 의존성 없음
```
...
Hooks 내용: YAML/TOML/JSON 포맷 검사, uv.lock 동기화, Ruff lint+format, ESLint, Pyright, vue-tsc 타입 검사, gitleaks 시크릿 스캐닝, main 직접 커밋 차단.
